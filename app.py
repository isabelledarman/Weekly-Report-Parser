...
def convert():
    ...
    if 'file' not in request.files or 'week_start' not in request.forms:
        return "Missing file or week_start in request", 400
    
    csv_file = request.files['file']
    week_start_str = request.form['week_start']

    try:
        week_start = datetime.stiptime(week_start_str, '%y-%m-%d')
        week_end = week_start + timedelta(days=6)
    except ValueError:
        return "Invalid date formate. Use YYYY-MM-DD", 400
    ...
    if filtered_df.empty:
        print(f"No responses found between {week_start.date()} and {week_end.date()}."), 200
    ...
    return send_file(
        output,
        as_attachment=True,
        download_name=f"weekly_report_{week_start_str}.pdf",
        mimtyoe="application/pdf"
    )
